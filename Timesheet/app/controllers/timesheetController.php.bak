...
class timesheetController extends Crayd_Controller {
	...
	$this->view->project = $this->db->fetchAll("SELECT DISTINCT Project FROM project");
	...
	if ($this->getRequest()->isPost()) {
		...
		$id = $this->user->id;
		$date = $data['date'];
		$sql = "SELECT * FROM leave_history WHERE $date BETWEEN from_date AND to_date AND employee_id = '$id'";
    ...
    public function submitAction() {
        ...
        foreach ($data as $key => $value) {
            $timesheetID = $key;
            echo $sql = "SELECT * FROM timesheet_detail WHERE timesheetID = '$timesheetID'";
            if ($this->db->count($sql) > 0) {
				...
				if ($projectID != '1') {
					echo $sql = "SELECT * FROM approval_project WHERE ProjectID = '$projectID'";
					if ($this->db->count($sql) > 0) {
