...
class historycanceledController extends Crayd_Controller {
	...
	public function listAction() {
		...
		if(($_REQUEST['from']) && ($_REQUEST['to'])){
			...
			$sql="SELECT a.*, b.*, c.*, d.*, e.name from timesheet_canceled a join project b on a.project=b.ProjectID 
			...
			$sql="SELECT a.*, b.*, c.*, d.*, e.name from timesheet_canceled a join project b on a.project=b.ProjectID 
			left join approval_project c on a.project=c.ProjectID 
			left join timesheet d on a.timesheet_id=d.timesheetID
			left join jml_users e on d.employeeID=e.employeeID
			where a.canceled_by='$user'
			$and
			$where
			...
			$sql="SELECT a.*, b.*, c.*, d.*, e.name emp_name, f.name approval from timesheet_canceled a join project b on a.project=b.ProjectID 
			left join approval_project c on a.project=c.ProjectID 
			left join timesheet d on a.timesheet_id=d.timesheetID
			left join jml_users e on d.employeeID=e.employeeID
			left join jml_users f on a.canceled_by=f.employeeID
	...
	public function cancelAction() {
	 ...
	 $delete_queue=$this->db->delete('timesheet_queue',"timesheet_id = '$timesheetID'");
	  //echo $delete_queue;
	 //$delete_approve=$this->db->delete('timesheet_approve',"timesheet_id = '$timesheetID'");
