...
class claimapprovalController extends Crayd_Controller { 
    ...
    public function listAction() {
       ...
       if ($_REQUEST['month']) {
		...
		if ($_REQUEST['project']) {
            $where .=" AND a.projectID = {$_REQUEST['project']} "; //ditambah ini 
        }
        ...
        $sql = " 
        SELECT a.*,b.name from claim a left join jml_users b on a.NIK=b.employeeID
		where a.status in ('Waiting Approval', 'ACTIVE')
		$where";
		...
		$sql = " 
        SELECT a.*,b.name from claim a left join jml_users b on a.NIK=b.employeeID
		where a.status in ('Waiting Approval', 'ACTIVE','PROBATION')
		$where 
		ORDER BY date DESC";
	...
	public function submitAction() {
		...
		$sql = "SELECT * FROM claim_approved where (approvedID) IN
				   (SELECT MAX(approvedID) FROM claim_approved)";

			if ($this->db->count($sql) > 0) {
				$result = $this->db->fetchRow($sql);
				$appid = $result['approvedID']+1;
			}
	    ...
	    $sql1 = "SELECT * FROM claim_approved where (approvedID) IN
				   (SELECT MAX(approvedID) FROM claim_approved)";

			if ($this->db->count($sql1) > 0) {
				$result1 = $this->db->fetchRow($sql1);
				$appidmax = $result1['approvedID'];
			}

	
		$sheets = $this->db->fetchAll("SELECT a.*, b.*,c.*,d.* from claim a join claim_approved b on a.claimID = b.claimID join jml_users c on a.id = c.id join project d on a.projectID = d.ProjectID where a.status = 'Approved' and b.approvedID between '$appid' and '$appidmax' GROUP BY c.id");
		...
		foreach ($sheets as $sheet) {
			...
			$sql2 = "SELECT SUM(days*value) AS Total from claim a join claim_approved b on a.claimID = b.claimID join jml_users c on a.id = c.id join project d on a.projectID = d.ProjectID where a.status = 'Approved' and b.approvedID between '$appid' and '$appidmax' and c.id = '$id'";

			if ($this->db->count($sql2) > 0) {
                $result = $this->db->fetchRow($sql2);
                $total = $result['Total'];
            }
			...
			$title  = "Pemberitahuan Project Allowance"; 
			// isi pesan email disertai password
			$pesan = "Dear $name, \n\nHari ini claim allowance anda sudah diapprove. Kami informasikan total claim allowance anda bulan ini sebesar Rp. $total. Demikian dapat kami informasikan.Terima kasih.\n\n\n\nRegards,\n\n\nRuth H Sinaga\n\n\n\n\n\n<<Pesan ini di kirim otomatis oleh spice.phincon.com>>";
			// header email berisi alamat pengirim
			$header = "From: ruth.sinaga@phintraco.com";
			// mengirim email
			$kirimEmail = mail($to, $title, $pesan, $header); 
		}    
    }
